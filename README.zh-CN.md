...
每个渠道通过临时自定义 provider 执行只读的 `codex exec --json` 调用，固定使用 Responses API 和 HTTPS，不启用 WebSocket，也不修改用户的 Codex 配置。API Key 保存后不会在界面回显。应用按渠道展示状态、成功/失败次数和最近结果；失败时完整展示原始 JSON 和错误输出。连续失败后恢复成功会弹出确认框，显示连续失败次数和成功时间，但不会停止 Loop。请求输出只保留在运行内存中，不写结果缓存或日志文件。Loop 不创建普通 AgentRadar 会话，退出 AgentRadar 时全部渠道自动停止。
