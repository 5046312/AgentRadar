...
- 内置 Codex Loop 可用性测试，支持多渠道并行检测、独立状态和统一随机间隔。
...
点击主弹窗中的试管按钮可打开独立 Loop 面板。面板支持配置多个渠道，每个渠道保存独立名称、Base URL 和 API Key；渠道编辑器可下载 TXT 模板，并使用单独一行 `---` 分隔后批量导入多个渠道。批量导入会先校验全部配置，成功或失败均显示系统提示。每个渠道可通过渠道行独立启动或停止。多个运行渠道彼此并行；每次启动立即执行首次检查，后续按该渠道上次结果在统一的成功/失败秒数范围内随机等待。

每个渠道通过临时自定义 provider 执行 `codex exec --json` 调用，固定使用 Responses API 和 HTTPS，不启用 WebSocket，也不修改用户的 Codex 配置。`--json` 固定保留；`--ephemeral`、`--ignore-rules`、禁用 hooks、只读 sandbox 和跳过 Git 仓库检查可在 Loop 设置中逐项启用。底部详情展示上次运行的完整脚本及 API Key，并提供复制按钮。应用按渠道展示状态、成功/失败次数和最近结果；失败时完整展示原始 JSON 和错误输出。连续失败后恢复成功会弹出确认框，显示连续失败次数和成功时间，但不会停止 Loop。请求输出和运行脚本只保留在运行内存中，不写结果缓存或日志文件。Loop 不创建普通 AgentRadar 会话，退出 AgentRadar 时全部渠道自动停止。
