...
点击主弹窗中的试管按钮可打开独立 Loop 面板。面板支持配置多个渠道，每个渠道保存独立名称、Base URL 和 API Key；渠道编辑器可下载 TXT 模板并导入配置。每个渠道可通过渠道行独立启动或停止。多个运行渠道彼此并行；每次启动立即执行首次检查，后续按该渠道上次结果在统一的成功/失败秒数范围内随机等待。

每个渠道通过临时自定义 provider 执行只读的 `codex exec --json` 调用，固定使用 Responses API 和 HTTPS，不启用 WebSocket，也不修改用户的 Codex 配置。API Key 保存后不会在界面回显。应用按渠道展示状态、成功/失败次数和最近结果；仅在失败后恢复成功时发送包含渠道名称的提示。Loop 不创建普通 AgentRadar 会话，退出 AgentRadar 时全部渠道自动停止。
