...
Click the test-tube button in the main popover to open the independent Loop panel. The panel supports multiple named channels, each with its own base URL and API key. The channel editor can download a TXT template and batch-import channels separated by a standalone `---` line. Batch imports validate every channel before writing and always show a native success or error message. Clicking a channel row starts or stops it independently. Running channels execute in parallel; each start runs its first check immediately, then waits within the shared success or failure interval selected from that channel's previous result.

Each channel invokes `codex exec --json` through a temporary custom provider. Requests use the Responses API over HTTPS with WebSockets disabled and do not modify the user's Codex configuration. `--json` is always enabled; `--ephemeral`, `--ignore-rules`, hooks disabling, the read-only sandbox, and the Git repository check bypass can each be enabled in Loop settings. The detail panel shows the complete last-run script, including the API key, with a copy button. AgentRadar shows per-channel state, success/failure counts, and the latest result; failures preserve the complete raw JSON and error output. Recovery after consecutive failures always opens a confirmation showing the failure count and success time without stopping the loop. Request output and displayed scripts stay in process memory and are not written to result caches or log files. Loop checks do not create normal AgentRadar sessions, and all channels stop when AgentRadar exits.
